<template>
  <div class="search-results">
    <div class="search-header">
      <h1 class="search-title">搜索结果：{{ route.query.q }}</h1>
      ...
      <p class="search-stats">
        找到 {{ total }} 个相关{{ searchType === 'video' ? '视频' : '用户' }}
      </p>
    </div>
    ...
    <el-empty
      v-if="
        !loading &&
        ((searchType === 'video' && searchResults.length === 0) ||
          (searchType === 'user' && userSearchResults.length === 0))
      "
...
<script setup lang="ts">
  ...
  const loading = computed(() =>
    searchType.value === 'video' ? videoStore.searchLoading : userLoading.value
  );
  const total = computed(() =>
    searchType.value === 'video' ? searchResults.value.length : userSearchResults.value.length
  );
  ...
  const fetchSearchResults = async () => {
    ...
    try {
      if (searchType.value === 'video') {
        ...
        userSearchResults.value = await searchUsers(searchKeyword.value);
      }
    } catch (error) {
